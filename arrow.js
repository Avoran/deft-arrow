angular.module('deft.arrow', [])
    .directive('arrow', function(){
        return {
            ...
            template: '' +
                '<div class="arrow" style="{{ transform(arrow.rotation) }}{{ arrowPosition() }}">' +
            ...
            link: function($scope){
                ...
                $scope.arrowPosition = function() {
                    switch ($scope.arrow.position) {
                        case 'bottom':
                        case 'top': return 'position:absolute;'+$scope.arrow.position+':-'+$scope.arrow.height+'px;left:50%;margin-left:-'+$scope.arrow.width/2+'px'; break;
                        case 'left':
                        case 'right': return 'position:absolute;'+$scope.arrow.position+':-'+($scope.arrow.width-$scope.arrow.height)+'px;top:50%;margin-top:-'+$scope.arrow.height/2+'px'; break;
                        default: return;
                    }
                };
